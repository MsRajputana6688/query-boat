...
const send_message = async (req, res) => {
    const { message, msgType, sender, receiver, time, chatFile } = req.body
    const rootDir = path.dirname(require.main.filename);
    try {
        const Newdata = { msgType, message, sender: sender, time, }
        if (msgType === 'text') {
            ...
            const Newdata = { msgType, message, sender: sender, time, file: file_path }
            saveDataInFile(Newdata, chatFile)
            file.mv(saveFile);
            res.status(200).json({
                file: file_path,
